...
Q.prototype.fail = function fail(task, optional_error, callback) {
  ...
  if (optional_error) task.error = optional_error instanceof Error ? optional_error.toString() : optional_error;
...
Q.prototype.listen = function rqlistener(opts) {
  ...
  sql.on('message', function (taskref, done) {
    async.waterfall([
      ...
      function (taskobj, cb) {
        if (!taskobj || !cb) {
          return cb(new Error('storage did not return a valid task object'))
        }
...
Q.prototype._finish_clean = function finish_clean(task, dontCheckFailed, callback) {
  ...
  async.auto({
    setTask: _.bind(this._keep_storage ? this.set : this.del, this, ref),
